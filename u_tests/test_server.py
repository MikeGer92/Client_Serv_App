...
class TestServerClass(unittest.TestCase):
    ...
    def test_no_action(self):
        ...
        self.assertNotEqual(process_client_message(
            {TIME: '1.1', USER: {ACCOUNT_NAME: 'Guest'}}), self.ok_dict)

    def test_wrong_action(self):
        """Ошибка если неизвестное действие"""
        self.assertNotEqual(process_client_message(
            {ACTION: 'Wrong', TIME: '1.1', USER: {ACCOUNT_NAME: 'Guest'}}), self.ok_dict)

    def test_no_time(self):
        """Ошибка, если  запрос не содержит штампа времени"""
        self.assertNotEqual(process_client_message(
            {ACTION: PRESENCE, USER: {ACCOUNT_NAME: 'Guest'}}), self.ok_dict)

    def test_no_user(self):
        """Ошибка - нет пользователя"""
        self.assertNotEqual(process_client_message(
            {ACTION: PRESENCE, TIME: '1.1'}), self.ok_dict)

    def test_unknown_user(self):
        """Ошибка - не Guest"""
        self.assertNotEqual(process_client_message(
            {ACTION: PRESENCE, TIME: 1.1, USER: {ACCOUNT_NAME: 'Guest1'}}), self.ok_dict)
